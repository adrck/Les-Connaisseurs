...
// own code - a defensive precaution in case createClient()'s internal
// detectSessionInUrl handling ever touches the hash before this script's
// next line runs. As it turned out, that wasn't actually the cause of the
// bug this was investigating (the real cause was a stray "hashchange"
// event overwriting the rendered confirmation screen - see the guard in
// main.js's handleHashNavigation()), but capturing this value up front,
// before anything else has a chance to touch the URL, is still the more
// robust pattern to keep regardless.
...
// the redirect hash does carry "...&token_type=bearer&type=signup". (The
// screen not showing was a separate bug entirely - see main.js's
// handleHashNavigation() guard.)
